...
public class TlsServerKeyExchangeMessage extends TlsHandshakeMessage {
    ...
    */
	 ...
	 * 
	 * @param dhParams the servers Diffie-Hellman parameters
	 * @param signedParams the signature of the parameters and the server and client random values
	 */
	public TlsServerKeyExchangeMessage(TlsServerDhParams dhParams, byte[] signedParams) {
		if (dhParams == null) {
			throw new IllegalArgumentException("DH parameters must be set!");
		}
		_dhParams = dhParams;
	...
	/**
	 * Creates an server key exchange message by parsing sent bytes.
	 * 
	...
	/**
	 * Returns the Diffie-Hellman parameters contained in this message.
	 * 
	...
	/**
	 * Returns the signature created with the servers private key of the
	 * parameters together with the random values.
	 * 
